...
void DocumentCommon::onCreateNewView()
{
  ...
  ws->addSubWindow (w);
  viewmap[w] = ws->subWindowList().size()-1;
...
{
    int i = viewmap[theView];
    removeView(theView);
    getApplication()->updateTreeWidgetItem();
    getApplication()->updatemysubWindows(i);
    viewmap.remove(theView);
    if( countOfWindow() == 0 )
...
int DocumentCommon::getmyIndex()
{
    return myIndex;
}

int DocumentCommon::getmyNbViews()
{
    return myNbViews;
}

QList<MDIWindow*> DocumentCommon::getmyViews()
{
    return myViews;
}
