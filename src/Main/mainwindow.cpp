...
#define PRINT_COST(ElapsedTimer,STR) \
    do{\
    int ___TMP_INT = ElapsedTimer.elapsed();\
    qDebug() << STR << ___TMP_INT/1000.0 << "(ms)";\
    }while(0)
...
MainWindow::MainWindow(QWidget *par):SARibbonMainWindow(par)
{
    ...
    m_edit = new QTextEdit(Lsplitter);
    //左窗口上下比例为1:2
    ...
    createFileMenu(ribbon->applitionButton());
...
void MainWindow::createFileMenu(QAbstractButton *menu)
{
    QList<QAction*> lists;
    QAction *stl = new QAction(".stl", this);
    QAction *stp = new QAction(".stp", this);
    QAction *igs = new QAction(".igs",this);

    QAction *import = new QAction("Import", this);
    import->setIcon(QIcon(":/icon/icon/openProject.png"));
    connect(import,&QAction::triggered,this,[this](bool b){this->m_edit->append("import action clicked");});
    connect(import,SIGNAL(triggered(bool)),this,SLOT(onImporttriggered(bool)));

    SARibbonMenu* mfile = new SARibbonMenu();
    SARibbonMenu* save_load = new SARibbonMenu();
    lists << stl << stp << igs; //读取配置页的3个选项
    save_load->addActions(lists);
    save_load->setTitle("Export");
    save_load->setIcon(QIcon(":/icon/icon/save.png"));
    mfile->addAction(import);
    mfile->addMenu(save_load);
    SARibbonApplicationButton *btn_file =dynamic_cast<SARibbonApplicationButton*>(menu);
    btn_file->setMenu(mfile);
}
...
void MainWindow::onImporttriggered(bool b)
{
    timer.restart();
    m_edit->append(QStringLiteral("onImporttriggered : %1").arg(b));
    FileReader filereader;
    Standard_Integer status =filereader.ReadIGES("F:/model/Femur.igs",project->aHSequenceOfShape);
    std::cout<<"return status:"<<status<<std::endl;
    PRINT_COST(timer,"read F:/model/Femur.igs");
}
